...
<%
	...
	String key = request.getParameter("payName");
...
<body>
    ...
    <div id="adproduct">
      <div class="outer">
        <h1>&nbsp;&nbsp;&nbsp;결제관리</h1>
        <br>
        <form action="<%=contextPath%>/paySearch.ad?currentPage=1" method="post">
        	아이디 <input type="text" name="payName"> <button type="submit" onclick="">조회</button><br>
        	<input type="hidden" name ="currentPage" value="1">
        </form>	
        <div id="allcount">
            <div id="countcontent">
                <div style="width:30%;">총 결제 : <input type="text" id="adproductcount" value="<%=pi.getListCount()%>"> 개</div>
               	<div style="width:70%; text-align:right; margin-left:330px;"><button onclick="location.href='<%=contextPath %>/paymentList.ad?currentPage=1'">전체리스트</button></div>
            </div>
        </div>  
       ...
       <div class="pagingArea" >
			<% if(currentPage!=1) {%>
			<button onclick="location.href='paySearch.ad?currentPage=1&payName=<%=key%>'"> &lt;&lt; </button>
			
			<button onclick="location.href='paySearch.ad?currentPage=<%=currentPage-1%>&payName=<%=key%>'"> &lt;</button>
			<% } %>
			
			<%for(int p=startPage; p<=endPage; p++){ %>
				<% if(currentPage != p){%>
				<button onclick="location.href='paySearch.ad?currentPage=<%=p%>&payName=<%=key%>'"><%=p%></button>
				<% }else { %>
			...
			<button onclick="location.href='paySearch.ad?currentPage=<%=currentPage+1%>&payName=<%=key%>'"> &gt;</button>
			<button onclick="location.href='paySearch.ad?currentPage=<%=maxPage%>&payName=<%=key%>'"> &gt;&gt; </button>
			<% } %>
		</div>
       </div>
    </div>
