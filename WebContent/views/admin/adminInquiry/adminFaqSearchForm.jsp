...
<%
	...
	String key = request.getParameter("faqCategory");
...
<body>
    ...
    <div id="adproduct">
      <div class="outer">
        <h1>&nbsp;&nbsp;&nbsp;FAQ 관리</h1>
        <br>
        	<form action="<%=contextPath%>/faqSearch.ad?currentPage=1" method="post">
        ...
        <div id="allcount">
                ...
                <div style="width:70%; text-align:right;"><button onclick="location.href='<%=contextPath%>/faqEnrollForm.ad'">작성하기</button>
                										<button onclick="location.href='<%=contextPath %>/faqList.ad?currentPage=1'">전체리스트</button>	
                 </div>
        </div>  
        <table class="listFaq">
            <thead>
       ...
       <div class="pagingArea" align="center">
			<% if(currentPage!=1) {%>
			<button onclick="location.href='faqSearch.ad?currentPage=1&faqCategory=<%=key%>'"> &lt;&lt; </button>
			
			<button onclick="location.href='faqSearch.ad?currentPage=<%=currentPage-1%>&faqCategory=<%=key%>';"> &lt;</button>
			<% } %>
			
			<%for(int p=startPage; p<=endPage; p++){ %>
				<% if(currentPage != p){%>
				<button onclick="location.href='faqSearch.ad?currentPage=<%=p%>&faqCategory=<%=key%>';"><%=p%></button>
				<% }else { %>
			...
			<button onclick="location.href='faqSearch.ad?currentPage=<%=currentPage+1%>&faqCategory=<%=key%>';"> &gt;</button>
			<button onclick="location.href='faqSearch.ad?currentPage=<%=maxPage%>&faqCategory=<%=key%>';"> &gt;&gt; </button>
			<% } %>
		</div>                  
