$(document).ready(function() {
	...
	$("#m_name").focusout(function(){
		...
		if(inputVal == ""){
			...
			//$("#m_name").focus();
			isChkId = false;
		} if(!overpass.validate.isValidName(inputVal)) {
			$('span[name=mbr_msg]').show();
	...
	$("#m_pw").focusout(function(){
		...
		if(inputVal.length < 10 && inputVal.length > 0 ){
			...
			isChkId = false;
		}else if(!overpass.validate.isValidPwd(inputVal) && inputVal.length > 0 ){
			$('span[name=pwd]').show();
	...
	$("#email").focusout(function() {
		...
		if(inputVal == ''){
			...
			isChkId = false;
		}else if(!overpass.validate.isValidEmail(inputVal)){
			$('span[name=email]').show();
    	...
    	$.ajax({
			...
			data : params,
			success: function(rs) {
				if(rs.result == "N" && email != $('#email').val()) {
	...
	//일자 추가
	fnGetLastDay = function(pin) {
		var lastDay = ( new Date( pin.year, pin.month, 0) ).getDate();
		
		var innerHTML = "<option value=''>선택</option>";
		var val;
		var checked;
		var daySelect = $("#" + pin.id);
		...
		for (var i = 1; i <= lastDay ; i++) {
			...
			if (Number(pin.selected) > 0 && val == Number(pin.selected)) {
				checked = "checked";
			}
			...
			innerHTML += "<option value='" + val + "' " + checked + ">" + val + "</option>";
		}
		
		daySelect.append(innerHTML);
	}
	...
	fninit = function () {
		...
		$("#cell_no1").attr("data-class", "phone_chk");
		$("#cell_no1").attr("title", "휴대폰 앞자리 선택");
		$("input[data-class='phone_chk']").attr("maxlength", 4);
