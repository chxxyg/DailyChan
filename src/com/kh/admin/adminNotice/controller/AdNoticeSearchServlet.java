...
/**
 * Servlet implementation class AdNoticeSearchServlet
 */
@WebServlet("/noticeSearch.ad")
public class AdNoticeSearchServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public AdNoticeSearchServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// * listCount : 총 게시글 갯수
		listCount = new AdNoticeService().adNoticeCount(noticeTitle);
		...
		ArrayList<Notice> list = new AdNoticeService().searchNotice(pi, noticeTitle);
		int count = new AdNoticeService().adNoticeListCount();
		
		// adminProductList페이지에 보내줌
		request.setAttribute("pi", pi);
		request.setAttribute("list", list);
		request.setAttribute("count", count);
		
		RequestDispatcher view = request.getRequestDispatcher("views/admin/adminInquiry/adminNoticeSearchForm.jsp");
